...
class Sequence(Dataset):
    ...
    def random_rotation(self, sample, max_angle=10):
        angle = random.uniform(-max_angle, max_angle)
        sample["event_volume"] = TF.rotate(sample["event_volume"], angle)
        if self.load_gt:
            sample["flow_gt"][0] = TF.rotate(sample["flow_gt"][0], angle)
            sample["flow_gt"][1] = TF.rotate(sample["flow_gt"][1], angle)
        return sample

    def add_gaussian_noise(self, sample, mean=0, std=0.1):
        noise = torch.normal(mean=mean, std=std, size=sample["event_volume"].size())
        sample["event_volume"] += noise
        return sample

    def event_dropout(self, sample, p=0.1):
        mask = torch.rand(sample["event_volume"].size()) > p
        sample["event_volume"] *= mask
        return sample
    ...
    def __getitem__(self, idx):
        ...
        if self.mode == "train":
            ...
            sample = self.event_dropout(sample)
    ...
    def get_voxel_grid(self, idx):
        ...
        if idx == 0:
            ...
            event_data = self.event_slicer.get_events(
                self.timestamps_flow[idx - 1],
                self.timestamps_flow[idx - 1] + self.delta_t_us,
            )
        else:
            raise IndexError
...
class SequenceRecurrent(Sequence):
    def __init__(
        ...
        super(SequenceRecurrent, self).__init__(
            seq_path,
            representation_type,
            mode,
            delta_t_ms,
